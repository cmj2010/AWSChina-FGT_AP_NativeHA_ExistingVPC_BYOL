resource "aws_iam_role" "iam-role1" {
  name = "${var.tag_name_prefix}-iam-role1"
...
resource "aws_iam_instance_profile" "iam-instance-profile1" {
  name = "${var.tag_name_prefix}-iam-instance-profile1"
  role = "${var.tag_name_prefix}-iam-role1"
}

resource "aws_iam_role_policy" "iam-role-policy" {
  name = "${var.tag_name_prefix}-iam-role-policy1"
  role = aws_iam_role.iam-role1.id
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
